...
export type StatTileColor = keyof typeof ICON_COLORS
export type StatTileVariant = 'centered' | 'horizontal'
export type StatTileTrend = 'up' | 'down' | 'neutral'

// Auto-cycle palette for grids
const COLOR_CYCLE: StatTileColor[] = ['brand', 'blue', 'purple', 'green', 'orange', 'pink']
...
// ─── Trend helpers ────────────────────────────────────────────────────
function getTrendInfo(trend: StatTileTrend, invertTrend: boolean) {
  const isUp = trend === 'up'
  ...
  const Icon: LucideIcon = isUp ? TrendingUp : isDown ? TrendingDown : ArrowUpRight
...
// ─── StatTileGrid ─────────────────────────────────────────────────────
const GRID_COLS: Record<StatTileGridColumns, string> = {
...
export interface StatTileMetric extends Omit<StatTileProps, 'color' | 'variant'> {
  key?: string | number
  color?: StatTileColor
}
