...
export type ContactAvatarSize = keyof typeof SIZE_CLASSES
...
export function getContactType(
  ...
  currentUserOrgType?: string | null
): ContactTypeKey {
 ...
 */
function UptradeLogo({ className, ...props }: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 100 100"
      className={className}
 ...
 */
function UptradeLogoBadge({ size = 'md' }: { size?: ContactAvatarSize }) {
  const badgeSizes: Record<ContactAvatarSize, string> = {
    xs: 'w-3 h-3',
    sm: 'w-3.5 h-3.5',
    md: 'w-4 h-4',
    lg: 'w-5 h-5',
 ...
 */
function LiveChatBadge({ isActive, className }: { isActive?: boolean; className?: string }) {
  return (
    <div className={cn(
